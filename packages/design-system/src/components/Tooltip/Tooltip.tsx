...
export interface TooltipProps {
  ...
  /**
   * When true, disable viewport-aware auto-flip and auto-shift. The tooltip
   * stays at the exact `placement` requested even if it overflows the viewport.
   * Useful for always-open positional labels (e.g. chart reference-line markers)
   * where the tooltip must stay aligned with a fixed coordinate.
   */
  lockPlacement?: boolean;
  /**
   * Override the tooltip's z-index. Tooltips default to a very high z-index
   * (10000) so hover tooltips stay above modals etc. For always-open contextual
   * labels that should respect the page stack (e.g. sit below a sticky nav),
   * pass a lower value such as `var(--z-index-above)`.
   */
...
function calcPos(
  ...
  // Flip to opposite side if overflowing on main axis (skipped when locked).
  if (!lockPlacement) {
  ...
  // Shift on cross-axis to stay within viewport (skipped when locked).
  if (!lockPlacement) {
...
export function Tooltip({
  ...
  const updatePos = useCallback(() => {
    ...
    setPos(
      calcPos(refRect, fl.offsetWidth, fl.offsetHeight, placement, off, lockPlacement),
    );
  }, [placement, offsetPx, showTail, anchor, setOpen, lockPlacement]);
  ...
  }, [isMounted, placement, offsetPx, showTail, anchor, lockPlacement]);
