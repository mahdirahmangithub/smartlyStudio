...
/** Reference-line vertical insets (chart-internal visual constants). */
const REF_LINE_TOP_INSET = 8; // matches --spacing-sm
const REF_LINE_BOTTOM_INSET = 0; // matches --spacing-2xs
...
export interface ChartRenderContext<D = any> {
  ...
  /** Series id hovered via the legend (null when none). */
...
export interface ChartContainerProps<D = any> {
  ...
  className?: string;
  ariaLabel?: string;
  /** Vertical reference lines drawn over the data area (e.g. "Today" markers). */
...
function ChartContainerInner<D>({
  ...
  const baseXScale = useMemo(() => {
    if (brushDomain) {
      ...
      return buildLinearScale(brushDomain as number[], innerWidth, false, [0, innerWidth]);
    }
    ...
    return buildLinearScale(allXValues as number[], innerWidth, true, [0, innerWidth]);
  ...
  const xScale = useMemo(() => {
    ...
    return buildLinearScale([x0 as number, x1 as number], innerWidth, false, [0, innerWidth]);
    ...
    >
      <Group left={effectiveMargin.left} top={effectiveMargin.top}>
        ...
        {referenceLines?.map((rl, i) => {
          const x = xScale(rl.x) ?? 0;
          const dash = rl.dashStyle ? DASH_PATTERNS[rl.dashStyle] : undefined;
          const color = rl.color ?? "var(--data-viz-warning-default)";
          // Square caps on the line (butt) so the bottom is flat, then a
          // small filled circle on the top end gives just that end a
          // rounded cap.
          return (
            <g key={`ref-${i}`}>
              <line
                x1={x}
                x2={x}
                y1={REF_LINE_TOP_INSET}
                y2={innerHeight - REF_LINE_BOTTOM_INSET}
                stroke={color}
                strokeWidth={1.5}
                strokeLinecap="butt"
                strokeDasharray={dash}
              />
              <circle cx={x} cy={REF_LINE_TOP_INSET} r={0.75} fill={color} />
            </g>
          );
        })}
