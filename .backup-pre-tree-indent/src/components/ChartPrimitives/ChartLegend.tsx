...
function LegendChips<D>({
  ...
  onToggle,
  onSeriesHover,
}: Pick<ChartLegendProps<D>, "series" | "hiddenSeries" | "onToggle" | "onSeriesHover">) {
  return (
    <>
      {series.map((s, i) => {
        ...
        return (
          <ToggleChip
            ...
            checked={!hidden}
            onChange={() => {
              if (!hidden) onSeriesHover?.(null);
              onToggle?.(s.id);
              if (hidden) onSeriesHover?.(s.id);
            }}
            onMouseEnter={() => !hidden && onSeriesHover?.(s.id)}
      ...
      <div className={styles.legendScrollOuter}>
        ...
        >
          <div className={styles.legendScrollInner} role="toolbar" aria-label="Chart legend">
            <LegendChips series={series} hiddenSeries={hiddenSeries} onToggle={onToggle} onSeriesHover={onSeriesHover} />
          </div>
        </ScrollFade>
