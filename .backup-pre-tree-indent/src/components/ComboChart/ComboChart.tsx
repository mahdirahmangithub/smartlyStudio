...
function ComboChartInner<D>({
  ...
  const getBarStyle = (catIdx: number, isCatHovered: boolean, seriesId: string): React.CSSProperties => {
    ...
    let opacity = 1;
    if (legendHoveredId && visibleSeries.length > 1) {
      opacity = seriesId === legendHoveredId ? 1 : 0.24;
    } else if (hoverCategory) {
          ...
          <Group left={margin.left} top={margin.top}>
            ...
            <g clipPath={`url(#${clipId})`}>
              ...
              {lineSeries.map((s, li) => {
                ...
                const legendDim = legendHoveredId && visibleSeries.length > 1 && legendHoveredId !== s.id;
                const chartDim = hoverCategory && hoverSeriesId && hoverSeriesId !== s.id;
                const hoverOpacity = legendDim || chartDim ? 0.24 : 1;
