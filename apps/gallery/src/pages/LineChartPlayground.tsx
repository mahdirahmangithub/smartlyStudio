...
import { LineChart as LineChartBase } from "@sds/components/LineChart";
import type { Series, LineChartProps, ChartAnnotation } from "@sds/components/LineChart";
import { setOklchEnhancement, isOklchEnhanced } from "@sds/components/ChartPrimitives";
import { Icon } from "@sds/components/Icon";
import { Card, CardContent, CardBody, CardTitle } from "@sds/components/Card";
...
function ForecastDemo() {
  const series = useMemo<Series<DataPoint>[]>(() => {
    // 31-day window. Cutoff sits ~5 days in so the actual segment is the
    // initial portion and the forecast extends across the rest.
    const today = new Date();
    const start = new Date(today.getTime() - 30 * 24 * 60 * 60 * 1000);
    const cutoff = new Date(start.getTime() + 5 * 24 * 60 * 60 * 1000);
    const data: DataPoint[] = [];
    let value = 100;
    for (let i = 0; i <= 30; i++) {
      // Slight upward drift; a steeper trajectory after the cutoff to make the
      // forecast obviously distinct from the actual trend.
      const drift = i < 5 ? 30 : 110;
      value = Math.max(0, value + drift + Math.random() * 30);
      data.push({
  ...
  const cutoff = series[0].forecastFrom as Date;
  ...
  return (
    ...
    <>
      ...
      <section style={sectionStyle}>
        ...
        <p style={{ fontSize: 13, margin: "0 0 8px", opacity: 0.7 }}>
          Per-series <code>forecastFrom</code> splits the line at a cutoff: solid before, dashed and lighter after.
          Combined with chart-level <code>referenceLines</code> for the &quot;Today&quot; marker.
        </p>
        <div style={cardStyle}>
          <ForecastDemo />
        </div>
